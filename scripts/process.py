...
if __name__ == '__main__':
  ...
  parser.add_argument('-nyc_fips_to_skip', '--nyc_fips_to_skip', type = list, dest = 'nyc_fips_to_skip', default = [36047, 36005, 36081, 36085], help = 'list of NYC fips to skip')
  ...
  NYC_counties = ['New York City','New York', 'Kings', 'Queens', 'Bronx', 'Richmond'] 
  ...
  for ind in selected_counties.index:
    ...
    if len(county.strip()) > 0:
      ...
      if county not in NYC_counties:
        ...
        #Require that county state matches expectation (there are counties with the same name in different states)
...
if(args.make_gif == 1):
  make_gif(counties_obj_list, 'df', 'total_deaths', '2020-01-21', '2020-04-20', args)
  make_gif(counties_obj_list, 'df', 'deaths_per_mil', '2020-01-21', '2020-04-20', args)
  make_gif_cv_days(counties_obj_list, 'cv_days_df_not_scaled', 'total_deaths', ndays, args, 1000)
  make_gif_cv_days(counties_obj_list, 'cv_days_df_per_mil', 'deaths_per_mil', ndays, args, 1000)
