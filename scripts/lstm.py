...
def create_df_list(df, args, seq_output):
  ...
  for i in range(len(df.index)):
    if i < args.min_entries_df_lstm: #only keep df if it has min number of entries
      continue
    if seq_output == 0:
...
def lstm(area, args, seq_output = 0):
  ...
  df = df[[args.name_cv_days, 'new_cases', 'new_deaths', 'total_cases', 'total_deaths']]
  ...
  model.add(LSTM(100, activation = 'relu', input_shape = (final_train_X.shape[1], final_train_X.shape[2])))
  if seq_output == 0:
    model.add(Dropout(0.2))
    model.add(Dense(1))

  else:
    model.add(RepeatVector(final_train_X.shape[1]))
